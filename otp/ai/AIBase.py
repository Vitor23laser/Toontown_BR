...
class AIBase:
    ...
    def restart(self):
        self.shutdown()
        self.taskMgr.add(self.___resetPrevTransform, 'resetPrevTransform', priority = -51)
        self.taskMgr.add(self.___ivalLoop, 'ivalLoop', priority = 20)
        self.taskMgr.add(self.___igLoop, 'igLoop', priority = 50)
        if self.AISleep >= 0:
            if not (self.AIRunningNetYield) or self.AIForceSleep:
                self.taskMgr.add(self.___sleepCycleTask, 'aiSleep', priority = 55)
